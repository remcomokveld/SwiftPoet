...
extension CodeWriter {
    public func emitFileHeader(o: AnyObject) {
        if let spec = o as? PoetSpec {

            let fileHeader = "//\n" +
            "//    \(spec.name).swift\n" +
            // Add Namespace/Framework here
            "//\n" +
            "// \(spec.generatedByAt()))\n" +
            "//\n" +
            "//\n\n"
            
            _out.appendContentsOf(fileHeader.characters)
        }
    }
    ...
    private func emitEmitter(o: Any?, first: Bool) {
        if let emitter = o as? Emitter {
            ...
            emitter.emit(self, asFile: false)
        }
    }
