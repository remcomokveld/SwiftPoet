...
while [[ $THIS_TRY < $MAXIMUM_TRIES ]]; do
	...
	( set -o pipefail && xcodebuild -project SwiftPoet.xcodeproj -configuration Release ONLY_ACTIVE_ARCH=YES -scheme SwiftPoet-$PLATFORM -destination "$DESTINATION" -destination-timeout 300 $XCODE_ACTION 2>&1 | tee "SwiftPoet-$PLATFORM-$OPERATION.log" | xcpretty )
